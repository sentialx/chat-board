...
export const stats = async (path: string): Promise<Stats | undefined> => {
  try {
...
//   await deletePath(...files.map((file) => join(path, file)));
// };

export const mkdirs = async (...paths: string[]): Promise<void> => {
  await Promise.all(
...
export const mkdirsSync = (...paths: string[]): void => {
...
export const replaceExtension = (path: string, newExt: string): string => {
  ...
  let p = removeExtension(basename(path)) + `.${newExt}`;
  if (dir !== ".") p = join(dir, p);
  return p;
};
