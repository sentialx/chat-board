...
export class ThreadScheduler<T = void> implements IThreadScheduler<T> {
  ...
  public getThreadByTask(
    task: ITask<T, ThreadExecutionContext<T>>,
  ): IThread<T> | undefined {
    return this.processedTasks.get(task);
  }

  public getThreadByTaskId(taskId: string): IThread<T> | undefined {
    const task = this.taskQueue.find((task) => task.getId() === taskId);
    if (task == null) return undefined;
    return this.getThreadByTask(task);
  }

  public getTaskByThread(
    thread: IThread<T>,
  ...
  public tick(): void {
    ...
    for (const task of taskQueueCopy) {
      ...
      if (thread == null && !this.searchAllThreads) return;
      // console.log(thread == null);

      // @TODO: Previously it breaked if thread was not found, so it may be a performance issue now.
      if (thread != null) {
        thread.lock(this);
