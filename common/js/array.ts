...
export const mergeMaps = <K, V>(a: Map<K, V>, b: Map<K, V>): Map<K, V> => {
  const map = new Map<K, V>(a);
...
export function filterByClass<T>(
  array: any[],
  cls: new (...args: any[]) => T,
): T[] {
  return array.filter((item) => item instanceof cls);
}
...
export function filterDuplicatesByKey<T, K extends keyof T>(
  array: T[],
  key: K,
): T[] {
  const map = new Map<T[K], T>();
